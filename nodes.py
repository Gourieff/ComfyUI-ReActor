...
class ReActorWeight:
    ...
    def set_weight(
        self, input_image, faceswap_weight, face_model=None, source_image=None
    ):
    ...
    def execute(
        ...
        if mask_optional is not None:
            ...
            if bbox_model_name not in self._bbox_cache:
                bbox_model_path = folder_paths.get_full_path(
                    "ultralytics", bbox_model_name
                )
            ...
            segs_all, seg_labels = bbox_detector.detect(
                image,
                bbox_threshold,
                bbox_dilation,
                bbox_crop_factor,
            ...
            if image.ndim == 4:
                ...
                for i in range(image.shape[0]):
                    ...
                    segs_tuple = (
                        ([segs_i], seg_labels)
                        if isinstance(segs_i, dict)
                        else (segs_i, seg_labels)
                    )
        ...
        if morphology_operation == "dilate":
            ...
            combined_mask = self.iterative_morphology(
                combined_mask, morphology_distance, op="dilate"
            )
            # print(f"after morph: {combined_mask.max()}, min: {combined_mask.min()}, sum: {combined_mask.sum()}")
        elif morphology_operation == "erode":
            combined_mask = self.iterative_morphology(
                combined_mask, morphology_distance, op="erode"
            )
        elif morphology_operation == "open":
            combined_mask = self.iterative_morphology(
                self.iterative_morphology(
                    combined_mask, morphology_distance, op="erode"
                ),
                morphology_distance,
                op="dilate",
        ...
        swapped_image = (
            swapped_image.to(device)
            if swapped_image.device != device
            else swapped_image
        )
        ...
        alpha_mask = (
            alpha_mask.to(device) if alpha_mask.device != device else alpha_mask
        )
        ...
        for i in range(0, MB):
            if is_empty[i]:
                ...
                paste_mask = (
                    paste_mask.to(device) if paste_mask.device != device else paste_mask
                )
